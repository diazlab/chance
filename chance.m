...
elseif strcmp(subr,'batch')
    %parameter file format: IP_file_name,Input_file_name,IP_sample_id,Input_sample_id,build,file_type
        ...
        f=fopen(outf,'a');
        fprintf(f,['IP\tInput\tIP_strength_test\tp-value\tFDR\t' ...
                   'IP_strength\tPercent_genome_enriched\tFDR_cancer_tfbs\tFDR_cancer_histone\t' ...
                   'FDR_normal_tfbs\tFDR_normal_histone\tFDR_comb\t' ...
                   'Input_bias_test\tInput_bias\tInput_bias_pvlaue\tip_scaling_factor\tinput_scaling_factor\tbatch\n']);
        ...
        for i=1:length(snrs.fdrs) %write the results to a tsv-file
            ...
            fprintf(f,'%g\t',abs(pvals{i}));
            fprintf(f,'%g\t',abs(mfdr));
            fprintf(f,'%g\t',abs(q{i}-p{i}));
            fprintf(f,'%g\t',abs(1-k{i}/m{i}));
            fprintf(f,'%g\t',abs(fd('tfbs_cancer')));
            fprintf(f,'%g\t',abs(fd('histone_cancer')));
            fprintf(f,'%g\t',abs(fd('tfbs_normal')));
            fprintf(f,'%g\t',abs(fd('histone_normal')));
            fprintf(f,'%g\t',abs(fd('all')));
            %            fprintf(f,'%g\t',enc_odz{i});
            %            fprintf(f,'%g\t',enc_p{i});
            if dip_p{i}<0.05, fprintf(f,'FAIL\t');
            else, fprintf(f,'PASS\t'); end
            fprintf(f,'%g\t',abs(dip{i}));
            fprintf(f,'%g\t',abs(dip_p{i}));
...
parfor i=1:length(s)
    ...
    s{i}=t;
    simh=sim_hist{i};smph=smp_hist{i};
    dl=[];pl=[];
    for j=1:100
        xr=samp_dist(abs(smph-simh),100);
        [dipt,p_valuet,~,~]=hartigansdipsigniftest(xr,simh,100);
        dl=[dl;dipt];pl=[pl;p_valuet];
    end
    dip{i}=median(dl);
    p_value{i}=median(pl);
end
...
function [t,fd,ht,k,m,sz_ip,sz_input,p,q,pval]=ip_strength(input_data,ip_data)
    ...
    if any(err==3)
        ...
        fd('all')=(1-normcdf(zval,nanmean(zvalnull(nx)),nanstd(zvalnull(nx))))/(2-normcdf(zval, ...
                  nanmean(zvalalt(ax)),nanstd(zvalalt(ax)))-normcdf(zval,nanmean(zvalnull(nx)),nanstd(zvalnull(nx))));
        ...
        fd('histone_cancer')=(1-normcdf(zval,nanmean(zvalnull_his_can(nx)),nanstd(zvalnull_his_can(nx))))/(2-normcdf(zval, ...
                                                          nanmean(zvalalt_his_can(ax)),nanstd(zvalalt_his_can(ax)))-normcdf(zval,nanmean(zvalnull_his_can(nx)),nanstd(zvalnull_his_can(nx))));;
        ...
        fd('histone_normal')=(1-normcdf(zval,nanmean(zvalnull_his_norm(nx)),nanstd(zvalnull_his_norm(nx))))/(1-normcdf(zval, ...
                                                          nanmean(zvalalt_his_norm(ax)),nanstd(zvalalt_his_norm(ax)))-normcdf(zval,nanmean(zvalnull_his_norm(nx)),nanstd(zvalnull_his_norm(nx))));;
        ...
        fd('tfbs_cancer')=(1-normcdf(zval,nanmean(zvalnull_tfbs_can(nx)),nanstd(zvalnull_tfbs_can(nx))))/(1-normcdf(zval, ...
                                                          nanmean(zvalalt_tfbs_can(ax)),nanstd(zvalalt_tfbs_can(ax)))-normcdf(zval,nanmean(zvalnull_tfbs_can(nx)),nanstd(zvalnull_tfbs_can(nx))));;
        ...
        fd('tfbs_normal')=(1-normcdf(zval,nanmean(zvalnull_tfbs_norm(nx)),nanstd(zvalnull_tfbs_norm(nx))))/(1-normcdf(zval, ...
                                                          nanmean(zvalalt_tfbs_norm(ax)),nanstd(zvalalt_tfbs_norm(ax)))-normcdf(zval,nanmean(zvalnull_tfbs_norm(nx)),nanstd(zvalnull_tfbs_norm(nx))));;
        fd('all')=max(0,min(1,fd('all')));
        fd('histone_cancer')=max(0,min(1,fd('histone_cancer')));
        fd('histone_normal')=max(0,min(1,fd('histone_normal')));
        fd('tfbs_cancer')=max(0,min(1,fd('tfbs_cancer')));
        fd('tfbs_normal')=max(0,min(1,fd('tfbs_normal')));
        fdflg=1; ht=1;
        for i=1:length(kz)
